    ...
    const res = await server.post("/api/hooks.unfurl", {
      body: {
        ...
        event: {
          type: "link_shared",
          channel: "Cxxxxxx",
          user: user.getAuth("slack").serviceId,
...
describe("#hooks.slack", () => {
  it("should return no matches", async () => {
    ...
    const res = await server.post("/api/hooks.slack", {
      body: {
        token: process.env.SLACK_VERIFICATION_TOKEN,
        user_id: user.getAuth("slack").serviceId,
        team_id: team.getAuth("slack").serviceId,
...
describe("#hooks.interactive", () => {
  it("should respond with replacement message", async () => {
    ...
    const payload = JSON.stringify({
      token: process.env.SLACK_VERIFICATION_TOKEN,
      user: { id: user.getAuth("slack").serviceId },
      team: { id: team.getAuth("slack").serviceId },
  ...
  it("should error if incorrect verification token", async () => {
    ...
    const payload = JSON.stringify({
      token: "wrong-verification-token",
      user: { id: user.getAuth("slack").serviceId, name: user.name },
